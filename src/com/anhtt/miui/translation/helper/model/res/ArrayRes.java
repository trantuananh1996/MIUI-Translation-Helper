...
public class ArrayRes implements Resource<ArrayRes> {
    ...
    public static ArrayRes create(Node element, boolean isApplyFilter) {
        ...
        if (isApplyFilter) isIgnoreValue = items.stream().allMatch(item -> item.getValue().contains("string/")
                ...
                || item.getValue().contains("@string/")
                || item.getValue().contains("@android:string/")
                || item.getValue().contains("sans-serif")
                || item.getValue().contains("google-sans")
                || item.getValue().equalsIgnoreCase("OK")
                || item.getValue().equals("999+")
                || item.getValue().equals("%d")
                || item.getValue().equals("%1$d")
                || item.getValue().equals("%2$d")
                || item.getValue().equals("%3$d")
                || item.getValue().equals("%s")
                || item.getValue().equals("%1$s")
                || item.getValue().equals("%2$s")
                || item.getValue().equals("%3$s")
                || item.getValue().equals("true")
                || item.getValue().equals("false")
                || (!containsHanScript(item.getValue()) && item.getValue().length() >= 15 && !item.getValue().contains(" "))
                || item.getValue().trim().replaceAll("\n", "").replaceAll("\t", "").length() == 0)
